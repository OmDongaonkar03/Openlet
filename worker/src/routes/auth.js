...
auth.post('/register', async (c) => {
  ...
  const { accessToken, user } = await issueTokens(c, {
    id: result.id, email: email.toLowerCase(), name,
  })
...
auth.post('/refresh', async (c) => {
  ...
  const blacklisted = await c.env.DB
    .prepare('SELECT 1 FROM refresh_token_blacklist WHERE token_hash = ?')
    .bind(tokenHash)
    .first()
  ...
  await c.env.DB
    .prepare('INSERT OR IGNORE INTO refresh_token_blacklist (token_hash, expires_at) VALUES (?, ?)')
  ...
  c.executionCtx.waitUntil(
    c.env.DB
      .prepare('DELETE FROM refresh_token_blacklist WHERE expires_at < ?')
