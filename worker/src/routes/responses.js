...
responses.post('/:slug', async (c) => {
  ...
  if (!turnstileToken) {
    return c.json({ error: 'Bot check token missing. Please refresh and try again.' }, 400)
  }
  ...
  if (!turnstileValid) {
    return c.json({ error: 'Bot check failed. Please refresh and try again.' }, 403)
  }
  ...
  if (ip !== 'unknown') {
    const ipDupe = await c.env.DB
      .prepare('SELECT id FROM submission_log WHERE page_id = ? AND ip = ?')
      .bind(page.id, ip)
      .first()
  ...
  if (fingerprint) {
    const fpDupe = await c.env.DB
      .prepare('SELECT id FROM submission_log WHERE page_id = ? AND fingerprint = ?')
